...
class ConversaService {
  ...
  static Future<void> salvarConversa(Conversa conversa) async {
    ...
    conversas.sort((a, b) => b.ultimaAtualizacao.compareTo(a.ultimaAtualizacao));
  ...
  static Future<List<Conversa>> listarConversasPorContexto(String contexto) async {
  ...
  static Future<String> gerarTituloAutomatico(
    ...
    final mensagensUsuario = mensagens
        .where((m) => m.isUser)
        .take(3)
        .map((m) => m.text)
        .join(' ');
...
''';
  ...
  static String _criarTituloFallback(String mensagens, String contexto) {
    final palavrasChave = [
      'equação', 'fração', 'geometria', 'álgebra', 'número',
      'soma', 'subtração', 'multiplicação', 'divisão', 'raiz',
      'potência', 'porcentagem', 'decimal', 'triângulo', 'círculo',
      'área', 'perímetro', 'volume', 'função', 'gráfico'
    ];
