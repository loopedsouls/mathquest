...
class ProgressoServiceV2 {
  ...
  static Future<void> _migrarDadosSeNecessario() async {
    ...
    if (!jaMigrado) {
      ...
      if (progressoJson != null) {
        try {
          final progressoAntigo = ProgressoUsuario.fromJsonString(progressoJson);
  ...
  static Future<void> _migrarEstatisticasModulos() async {
    final prefs = await SharedPreferences.getInstance();
    final keys = prefs.getKeys().where((key) => key.startsWith('estatisticas_modulo_'));
  ...
  static Future<void> _atualizarEstatisticasModulo(String unidade, String ano, bool correta) async {
    final stats = await DatabaseService.carregarEstatisticasModulo(
      unidade: unidade,
      ano: ano,
    );
  ...
  static Future<Map<String, dynamic>> obterInformacoesModulo(String unidade, String ano) async {
    final progresso = await carregarProgresso();
